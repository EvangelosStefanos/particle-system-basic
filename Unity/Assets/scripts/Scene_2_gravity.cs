...
namespace Scenes
{
    public class Scene_2_gravity : MonoBehaviour
    {
        MyParticleSystem ps;
        ...
        void Start()
        {
            ...
            setColor(go_1, Color.red);

            GameObject go_2 = createGo("fixed_"+2);
            go_2.transform.position = new Vector3(2, 0, 0);
            setColor(go_2, Color.blue);

            forces.Add(new GravityNt(new List<Particle>(new[]{
                new Particle(1, new Vector3(-2, 0, 0), Vector3.zero, Vector3.zero),
                new Particle(1, new Vector3(2, 0, 0), Vector3.zero, Vector3.zero)
            }), "attract"));
